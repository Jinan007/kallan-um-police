...
## Android APK (Capacitor) - done
- Capacitor 8 wraps the built game (`webDir: dist`, appId `com.kallanumpolice.app`, `capacitor.config.ts`). The `android/` project is committed; `android/app/src/main/assets/public` is generated by `cap sync` and git-ignored.
- Branding: launcher icons (mipmap-*, adaptive foreground on #3B2A1A), a flat dark splash (`splash.png` files are plain #1C1108, `styles.xml` splash theme), dark status/navigation bars, portrait lock in AndroidManifest. Icon sources: `public/icons/apple.svg` (legacy square/round) and `scripts/icons/android-foreground.svg` (adaptive foreground); PNGs were rendered from them with a browser canvas.
- `.github/workflows/android-apk.yml`: on push to main (or manual), Node 22 + JDK 21, `npm ci`, `npm run build`, `cap sync`, `./gradlew assembleDebug`, then a rolling GitHub Release tagged `apk-latest` with `kallan-um-police.apk` (also an Actions artifact). Debug-signed, so it installs by sideloading but is not Play-Store-ready (that needs a release keystore and an AAB).
- `useInstall` hides the install card inside the native app (`Capacitor.isNativePlatform()`).
- Not verified: the APK has not been run on a real phone or emulator by me; only that it built and contains the game, service worker, manifest and icons.
